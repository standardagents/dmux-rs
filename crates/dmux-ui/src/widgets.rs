...
pub fn draw_button(
    ...
    let (fg, bg, attrs) = match (style, focused) {
        (ButtonStyle::Primary, true) => (dmux_compositor::Color::Indexed(233), theme.accent, AttrFlags::BOLD),
        (ButtonStyle::Primary, false) => (theme.accent, theme.bg_selected, AttrFlags::BOLD),
        (ButtonStyle::Danger, true) => (dmux_compositor::Color::Indexed(233), theme.danger, AttrFlags::BOLD),
...
pub fn draw_kv_row(
    ...
    buf.draw_text(rect.x + 1, rect.y, label, label_fg, bg, if selected { AttrFlags::BOLD } else { AttrFlags::empty() }, rect);
    let value_fg = if enabled { theme.accent } else { theme.text_faint };
...
pub fn draw_hint_bar(buf: &mut CellBuffer, rect: Rect, hints: &[(&str, &str)], theme: &Theme) {
    ...
    for (i, (key, desc)) in hints.iter().enumerate() {
        if i > 0 {
            x = buf.draw_text(x, rect.y, "  ·  ", theme.text_faint, bg, AttrFlags::empty(), rect);
        }
        ...
        x = buf.draw_text(x, rect.y, desc, theme.text_dim, bg, AttrFlags::empty(), rect);
    }
}
